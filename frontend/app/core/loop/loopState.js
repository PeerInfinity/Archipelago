...
import { proposedLinearReduction, proposedLinearFinalCost } from './xpFormulas.js';
...
class LoopState {
  ...
  queueAction(action) {
    ...
    if (action.regionName) {
      ...
      const lastIndex = this.actionQueue.map(a => a.regionName).lastIndexOf(action.regionName);
      if (lastIndex !== -1) {
  ...
  _processFrame(timestamp) {
    ...
    if (this.currentAction) {
      ...
      if (Math.random() < 0.05) {
        console.log(`Action progress: ${this.currentAction.progress.toFixed(2)}%, Mana: ${this.currentMana.toFixed(2)}/${this.maxMana}`);
      }
  ...
  _completeCurrentAction() {
    ...
    if (completedAction.type === 'explore' && completedAction.repeatAction) {
      console.log(`Repeating explore action for ${completedAction.regionName} (repeatAction=${completedAction.repeatAction})`);
  ...
  _handleExploreCompletion(action) {
    ...
    if (undiscoveredLocations + undiscoveredExits > 0) {
      ...
      const revealLocation = Math.random() * (undiscoveredLocations + undiscoveredExits) < undiscoveredLocations;
  ...
  _handleLocationCheckCompletion(action) {
    ...
    const location = stateManager.locations.find(loc => loc.name === locationName);
  ...
  setAutoRestartQueue(autoRestart) {
    ...
    eventBus.publish('loopState:autoRestartChanged', { autoRestart: this.autoRestartQueue });
  }
  ...
  restartQueueFromBeginning() {
    ...
    regions.forEach(region => {
      ...
      const moveActions = this.actionQueue.filter(a => 
        a.type === 'moveToRegion' && a.destinationRegion === region
      );
      sortedActions.push(...moveActions);
      
      // Then add all actions in this region
      const regionActions = this.actionQueue.filter(a => 
        a.regionName === region && a.type !== 'moveToRegion'
      );
      sortedActions.push(...regionActions);
    });
    
    // Add any remaining actions not captured by region sorting
    const remainingActions = this.actionQueue.filter(a => !sortedActions.includes(a));
  ...
  saveToStorage() {
    try {
      ...
      localStorage.setItem('archipidle_loop_state', JSON.stringify(serializedState));
    } catch (error) {
